...
enum Upscayl {
    ...
    static func process(
        ...
        if FileManager.default.fileExists(atPath: newURL.path) && !(HiPixelConfiguration.shared.overwritePreviousUpscale) {
        ...
        if result.status == 0 && HiPixelConfiguration.shared.imageCompression > 0 {
            if HiPixelConfiguration.shared.enableZipicCompression && AppInstallationChecker.isAppInstalled(bundleIdentifier: "studio.5km.zipic") {
                // Use Zipic for compression
                let saveDir = HiPixelConfiguration.shared.enableSaveOutputFolder ? URL(string: HiPixelConfiguration.shared.saveOutputFolder ?? "")?.standardizedFileURL : nil
    ...
    static func process(
        ...
        for url in urls {
            if url.hasDirectoryPath {
                let imageURLs = Common.getImageURLs(at: url)
        ...
        for signURL in signURLs {
            ...
            let operation = BlockOperation {
                ...
                if url != nil {
                    ...
                    do {
                        try fileManager.createDirectory(at: _saveDir, withIntermediateDirectories: true, attributes: nil)
                    } catch {
                ...
                if !Common.fileExists(at: imageURL) {
                    return
                }
        ...
        group.notify(queue: DispatchQueue.main) {
            if HiPixelConfiguration.shared.notification != .None {
                NotificationX.push(message: String(format: NSLocalizedString("Upscale completed: %d images", comment: ""), signURLs.count))
            }
        }
    }
    ...
    private static func makeOutputURL(for item: UpscaylDataItem, ext: String) -> URL {
        var url = item.url
        if HiPixelConfiguration.shared.enableSaveOutputFolder, let saveFolder = HiPixelConfiguration.shared.saveOutputFolder, let baseDir = URL(string: "file://" + saveFolder) {
            url = baseDir.appendingPathComponent(url.lastPathComponent)
        }

        let postfix = "_hipixel_\(Int(HiPixelConfiguration.shared.imageScale))x_\(HiPixelConfiguration.shared.upscaleModel.id)"
